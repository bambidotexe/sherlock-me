...
/// The app's two requests to the network, and the only ones it ever makes: GitHub's latest release, and
/// that release's disk image. The first is made shortly after launch, weekly after that, and when the
/// Settings button asks; the second only ever after a click on Update.
///
/// Completions are called on the session's queue, never on the main actor. The caller hops.
public enum UpdateChecker {
    /// `SHERLOCKME_UPDATE_FEED` points a build at a stand-in for GitHub's reply: a `file://` or `http://`
    ...
    public static var feedURL: URL {
        ProcessInfo.processInfo.environment["SHERLOCKME_UPDATE_FEED"].flatMap(URL.init(string:))
            ?? UpdateCheck.latestReleaseAPI
    }

    /// Asks GitHub for the repository's latest release and reports what it means. The cache is bypassed: a
    /// user pressing the button a second time is asking about now, not about the reply that came ten
    /// minutes ago.
    public static func check(current: String, session: URLSession = .shared,
        ...
        var request = URLRequest(url: feedURL,
                                 ...
                                 timeoutInterval: K.updateCheckTimeout)
...
/// GitHub said about the asset (its length, its SHA-256) before anyone is told it is there: a download cut
/// short or altered on the way never reaches the disk image tools. Both closures are called on the
...
public final class UpdateDownload: NSObject, URLSessionDownloadDelegate, Sendable {
    ...
    /// Whatever GitHub stated about the asset has to hold; what it did not state is not held against the
    /// file.
