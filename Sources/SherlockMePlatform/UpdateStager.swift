...
/// Takes the app out of a release's disk image and decides whether it may replace the running one, all of
/// it while the running one is still up: by the time "Install and Relaunch" is enabled, the copy that will
/// be moved into place is on the same volume, is this app, is newer, runs on this macOS and is signed by
/// the same team. Blocking: call it off the main thread.
public struct UpdateStager: Sendable {
    ...
    public func stage(diskImage: URL, in directory: URL) throws -> URL {
        ...
        guard let source = candidates.first(where: { Self.facts(of: $0).bundleIdentifier == bundleIdentifier })
        else { throw StagingError.appMissing }
    ...
    /// Read-only, hidden from the Finder, at a mount point of our own. `hdiutil` first, which every macOS
    /// this app runs on has; `diskutil image`, its successor, where `hdiutil` no longer does it.
    ...
    /// Only ever asked of a folder something is mounted on: a plain folder's path names the volume it sits
    /// on, and that volume is the Mac's own.
    ...
    private static func isMountPoint(_ url: URL) -> Bool {
        ...
        guard stat(url.path, &here) == 0, stat(url.deletingLastPathComponent().path, &parent) == 0
        else { return false }
