...
/// so nothing here touches it unless the app runs from one.
@MainActor
final class UpdateNotifier: NSObject, UNUserNotificationCenterDelegate {
    ...
    func start() {
        ...
        let update = UNNotificationAction(identifier: UpdateNotification.action,
                                          title: Loc.updateWindow.updateAction, options: [.foreground])
        center.setNotificationCategories([
            UNNotificationCategory(identifier: UpdateNotification.category, actions: [update],
                                   intentIdentifiers: []),
        ])
    }
    ...
    func postUpdateAvailable(version: String) {
        ...
        center.requestAuthorization(options: [.alert]) { granted, _ in
            guard granted else {
                Log.update.notice("notification not posted: not allowed; the release shows in Settings")
                return
            }
            ...
            UNUserNotificationCenter.current().add(
                UNNotificationRequest(identifier: UpdateNotification.identifier, content: content,
                                      trigger: nil))
        }
    }
    ...
    /// Asked only while the app is frontmost, which for this app means one of its windows is in front: the
    /// notification shows then too.
    nonisolated func userNotificationCenter(_ center: UNUserNotificationCenter,
