...
/// fetches the release and makes it ready while the app is still running; "Install and Relaunch" then hands
/// two folders to a helper and quits the app the way the menu's Quit does.
///
/// The rules are `SherlockMeCore`'s (`UpdatePanel`, `UpdateSession`, `StagedUpdateCheck`,
/// `UpdateInstallScript`); this object runs the requests and words the answers.
@MainActor
final class UpdateController: ObservableObject {
    ...
    let appVersion = AppIdentity.version
    ...
    var outcomeIsWaiting: Bool {
        guard let written = (try? FileManager.default.attributesOfItem(atPath: Self.resultFile.path)[.modificationDate]) as? Date
        else { return false }
    ...
    private static let stagingQueue = DispatchQueue(label: "\(AppIdentity.bundleIdentifier).update.staging",
                                                    qos: .userInitiated)
    ...
    func start() {
        ...
        notifier.start()
        readLastInstall()
        timer = Timer.scheduledTimer(withTimeInterval: K.updateLaunchDelay, repeats: false) { [weak self] _ in
            MainActor.assumeIsolated {
                guard let self else { return }
                self.tick()
                self.timer = Timer.scheduledTimer(withTimeInterval: K.updateTick, repeats: true) { [weak self] _ in
    ...
    private func readLastInstall() {
        ...
        guard let written, UpdateResult.isNews(age: Date().timeIntervalSince(written)) else {
            Log.update.notice("an install result left behind is ignored: \(line, privacy: .public)")
            return
        }
        switch UpdateResult(line: line) {
        case .installed(let version):
            Log.update.notice("version \(version, privacy: .public) installed")
            present(.installed(version: version))
        case .failed(let version, let reason):
            Log.update.error("version \(version, privacy: .public) NOT installed (\(reason.rawValue, privacy: .public)); still \(self.appVersion, privacy: .public)")
            panel.installFailed(Self.words(for: reason))
            present(.failed(version: version, reason: reason))
        case nil:
            Log.update.error("unreadable install result: \(line, privacy: .public)")
        }
    }
    ...
    private func checkEnded(_ result: Result<UpdateDecision, Error>, asked wasAsked: Bool) {
        checkInFlight = false
        let asked = wasAsked || answerIsAwaited
        answerIsAwaited = false
        switch result {
        case .success(let decision):
            schedule.answered(at: Date())
            Log.update.notice("check (\(asked ? "asked" : "automatic", privacy: .public)): \(Self.logLine(decision), privacy: .public)")
            if asked {
        ...
        case .failure(let error):
            ...
            Log.update.notice("check (\(asked ? "asked" : "automatic", privacy: .public)) failed: \(error.localizedDescription, privacy: .public)")
    ...
    /// The notification's Update, and a click on the notification itself. Same thing as the Settings button
    /// once it reads Update; a notification left by an earlier run asks GitHub first, then opens the window
    /// on the answer.
    func presentUpdate() {
    ...
    private func startDownload(_ release: LatestRelease) {
        ...
        let destination = Self.directory
            .appendingPathComponent("\(AppIdentity.name)-\(release.version.displayString).dmg")
        Log.update.notice("downloading \(release.dmgURL.absoluteString, privacy: .public)")
    ...
    private func downloadEnded(_ result: Result<URL, Error>) {
        ...
        case .success(let image):
            Log.update.notice("downloaded \(image.lastPathComponent, privacy: .public)")
            diskImage = image
            session?.downloaded()
            if let obstacle = UpdateInstaller.obstacle(updatesDirectory: Self.directory) {
                Log.update.notice("the app cannot replace itself here (\(obstacle.rawValue, privacy: .public)); the disk image is the way")
    ...
    /// Unpacks and checks the release off the main actor; only a copy that passed everything enables the
    /// button.
    private func prepare(_ image: URL) {
        let current = generation
        let directory = Self.directory
        let stager = UpdateStager(bundleIdentifier: AppIdentity.bundleIdentifier, runningVersion: appVersion,
                                  log: { Log.update.notice("\($0, privacy: .public)") })
        Self.stagingQueue.async { [weak self] in
            ...
            Task { @MainActor in
                ...
                case .success(let staged):
                    ...
                    Log.update.notice("ready to install")
                case .failure(let error):
                    Log.update.error("preparing failed: \(String(describing: error), privacy: .public)")
    ...
    /// The way out when the app cannot replace itself: macOS mounts the image and shows its Applications
    /// link.
    func openDiskImage() {
        guard let image = diskImage else { return }
        Log.update.notice("opening \(image.lastPathComponent, privacy: .public)")
    ...
    private func sweep() {
        ...
        where item.pathExtension == "dmg" || item.lastPathComponent == "staged"
            || item.lastPathComponent == "install.sh" {
    ...
    func installAndRelaunch() {
        ...
        guard FileManager.default.fileExists(atPath: staged.path),
            ...
            Log.update.error("the prepared copy is gone or the app can no longer be replaced")
        ...
        let plan = UpdateInstallPlan(
            ...
            executableName: Bundle.main.executableURL?.lastPathComponent ?? AppIdentity.name,
        ...
        do {
            ...
            Log.update.error("the install helper could not be started: \(String(describing: error), privacy: .public)")
            session?.failed(Self.words(for: .replace))
            return
        }
        _ = session?.install()
        // The helper opens the new version, and an opened bundle is how a person asks for the Settings
        // window. Nobody asked for one here: the marker says this launch is not a person's.
        QuietLaunch.mark()
        Log.update.notice("installing \(plan.version, privacy: .public); quitting")
        // An app that is still here after `updateStallNotice` stops the helper before it says so, so that a
        // quit that comes later is only ever a quit. What was prepared is still good.
        DispatchQueue.main.asyncAfter(deadline: .now() + K.updateStallNotice) { [weak self] in
            MainActor.assumeIsolated {
                guard let self, self.session?.phase == .installing else { return }
                if let helper = self.helper { UpdateInstaller.stop(helper) }
                self.helper = nil
                Log.update.error("the app did not quit; helper stopped, install abandoned")
