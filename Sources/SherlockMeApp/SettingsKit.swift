// The kit every settings page is built from: SwiftUI only, nothing app-specific. This file is the family's
// reference copy, and the `macos-building-settings-pages` skill is its contract; the numbers in
// `SettingsMetrics` were fitted by eye by the owner, round after round, and are not an agent's to retune.
//
// A page is a column of groups. A group is a title, a card of rows, and under the card, OUTSIDE it, a hint,
// then warnings, then notes. A row is a control and its label and nothing else.
...
enum SettingsMetrics {
    ...
    /// A slider's track, and the column its value is printed in.
...
/// A number the user can move, with its value printed beside it in fixed-width digits. The owner chose it
/// for KoffeeLid's effect tunables; a new slider is still the owner's to decide.
struct SliderRow: View {
    ...
    init(_ title: String, value: Binding<Double>, in range: ClosedRange<Double>, step: Double,
         enabled: Bool = true, format: @escaping (Double) -> String) {
        self.title = title
        self.value = value
        self.range = range
        self.step = step
    ...
    var body: some View {
        SettingsRow(title, enabled: enabled) {
            HStack(spacing: SettingsMetrics.rowSpacing) {
                ...
                // Fixed-width digits, so the number does not shuffle while it is dragged. It dims with
                // the label: a disabled slider's value is not a live reading.
                Text(format(value.wrappedValue))
                    .font(.body)
                    .monospacedDigit()
                    .foregroundStyle(enabled ? Color.primary : Color.secondary)
                    .frame(width: SettingsMetrics.valueWidth, alignment: .trailing)
            }
        }
    }
}

/// A choice among named options too many or too long for a segmented control, as a pop-up menu at the
/// trailing edge. The owner chose it for KoffeeLid's six lid-close sounds; a new choice is still the
/// owner's to decide, never an agent's.
struct PopUpRow<Option: Hashable>: View {
    private let title: String
    private let options: [Option]
    private let selection: Binding<Option>
    private let enabled: Bool
