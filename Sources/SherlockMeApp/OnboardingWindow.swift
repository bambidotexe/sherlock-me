...
/// The first-run wizard. This file is the family's reference copy; read the `macos-building-onboarding`
/// skill whole before changing anything here: most of it is traps, and every one of them shipped once.
///
/// Three pages: the pitch, where the app lives, and "All set". An app that needs a permission puts one list
/// page per set of grants between the first two; the skill says how.
///
...
/// its turn like any other: a permission dialog and System Settings both open over it and stay there until
/// the user leaves them. It belongs to the Space it opened in and keeps its place in it across a Space
...
/// the app, through `grantMayHaveChanged`, so an app that needs a permission starts the moment it arrives. A
/// page is built only on a change of step; a row that moves redraws its own trailing control and nothing
/// else.
@MainActor
final class OnboardingWindowController: NSWindowController, NSWindowDelegate {
    ...
    /// Called on every poll tick and whenever a row's flow ends, so the app can start or stop what a grant
    ...
    /// is how a grant is acted on without a second timer. A page with no rows (the pitch, "All set") still
    /// reports: a grant can arrive while either is on screen.
    ...
    static func make(store: SettingsStore, onFinish: @escaping () -> Void,
        ...
        let pages: [OnboardingPage] = [
            .hero(title: words.pitchHeadline,
                  ...
                  pills: [Pill(symbol: "menubar.rectangle", text: words.pitchMenuBarPill),
                          Pill(symbol: "lock.shield", text: words.pitchPrivatePill)],
...
enum OnboardingMetrics {
    ...
    /// A list page of one or two rows.
