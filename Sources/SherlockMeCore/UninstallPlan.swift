...
/// What taking the app off a Mac has to remove, and the part of it that can only happen once this process
/// has gone.
///
...
public enum UninstallPlan {
    ...
    /// What is left of the app after an uninstall, for a check that it really has gone: the bundle, the
    ...
    /// whatever happens, leaving an empty plist where a Mac that never had the app has no file at all. So
