...
public struct GeneralPageStrings {
    ...
    public var uninstallHint: String {
        switch language {
        case .en: "Removes the entry in Login Items, and removes \(AppIdentity.name)'s settings and its "
            + "update folder. \(AppIdentity.name) then moves itself to the Trash and quits."
        case .fr: "Retire l'entrée dans les éléments d'ouverture de session, et supprime les réglages de "
            + "\(AppIdentity.name) et son dossier de mises à jour. \(AppIdentity.name) se met ensuite à la "
            + "corbeille et quitte."
        }
    }
    ...
    public var uninstallWarning: String {
        switch language {
        case .en: "Do not drag \(AppIdentity.name) to the Trash. The Login Items entry stays behind, "
            + "pointing at an app that is gone."
        case .fr: "Ne mettez pas \(AppIdentity.name) à la corbeille vous-même. L'entrée dans les "
            + "éléments d'ouverture de session resterait en place, pointant vers une app disparue."
        }
    }
    ...
    public var uninstallConfirmBody: String {
        switch language {
        case .en: "\(AppIdentity.name) removes the entry in Login Items, and removes its settings and its "
            + "update folder. It then moves itself to the Trash and quits."
        case .fr: "\(AppIdentity.name) retire l'entrée dans les éléments d'ouverture de session, et "
            + "supprime ses réglages et son dossier de mises à jour. Il se met ensuite à la corbeille et "
            + "quitte."
        }
    }
