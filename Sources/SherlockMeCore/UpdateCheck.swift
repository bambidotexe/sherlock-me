...
/// A dotted version — "1.7.0", or "v1.7.0" as a git tag writes it — compared numerically per component
/// rather than as text, so "1.0.10" is newer than "1.0.9". A missing trailing component counts as 0, which
/// makes "1.7" and "1.7.0" the same version.
...
/// release has to meet to be offered at all: a tag that parses as a version, and an asset whose name ends
/// in `.dmg`. A release that meets neither is read as no release rather than as a broken one.
///
/// The asset's length and SHA-256 are GitHub's own statement about the file it serves. The length sizes the
/// progress bar when the download's response carries none, and a finished download is held against both
...
public struct LatestRelease: Equatable, Sendable {
    ...
    /// nil on malformed JSON, on a missing or unparsable `tag_name`, and on a release with no asset ending
    /// in `.dmg`. The first such asset wins; a release carrying several is not a case the build script can
    /// produce.
...
/// The update check's whole decision: what to ask GitHub, and what an answer means. Making the request is
/// `SherlockMePlatform.UpdateChecker`'s job; nothing here touches the network, so every rule below is
...
public enum UpdateCheck {
    ...
    public static var latestReleaseAPI: URL {
        URL(string: "https://api.github.com/repos/\(AppIdentity.repository)/releases/latest")!
    }

    /// `available` only for a release **strictly** newer than what is running. An equal or older release is
    /// up to date, and so is a `current` that does not parse: a binary run outside its bundle has no
    /// version, and offering that an update would be offering it to everything.
    ...
    /// What one reply from `latestReleaseAPI` means, read by its status first. 404 is GitHub's answer for a
    /// repository that has published no release — and for one it will not show an anonymous caller at all,
    /// which it deliberately does not tell apart; both are reported as having nothing to offer rather than
    /// as a failure.
