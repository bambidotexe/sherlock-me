#!/bin/sh
# Assembles build/SherlockMe.app from the release build and signs it with the Wooflab team's Developer ID,
...
# System Settings > Language & Region's per-app list, which is how a French Mac can be told to show this one
# in English.
...
if [ "$SIGN_IDENTITY" = "-" ]; then
    # Ad-hoc: it cannot be notarized, and every build gives the app a new code identity, so macOS forgets
    # every grant the owner has already given the app. It exists to read something a signed build will not
    # show, and it is never made unless the owner has asked for one: DEBUG_OK=1 is how the caller says so.
    ...
    echo "         code identity, so every permission grant is lost. Ship with scripts/release.sh." >&2
    SIGN_FLAGS=""
else
