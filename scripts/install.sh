...
# (Sources/SherlockMeCore/QuietLaunch.swift). It lapses on its own after two minutes.
...
# The running copy goes before its bundle is replaced: two instances would fight over the same preferences
...
# A stable Developer ID identity keeps the same code signature across installs, so every grant the app was
# given in System Settings survives this reinstall; an ad-hoc build would lose them every time.
open "$DEST"
