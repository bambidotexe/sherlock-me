...
final class CrashReportsTests: XCTestCase {
    ...
    func testOnlyRecentReportsOfThisProcessAreCountedNewestFirst() throws {
        let now = Date()
        try write("SherlockMe-2026-09-20-101010.ips", at: now.addingTimeInterval(-3_600))
        try write("SherlockMe-2026-09-21-101010.ips", at: now.addingTimeInterval(-60))
        try write("SherlockMe-2026-09-01-101010.ips", at: now.addingTimeInterval(-20 * 86_400))
        try write("ExcUserFault_SherlockMe-2026-09-21-101010.ips", at: now)
        try write("Other-2026-09-21-101010.ips", at: now)

        let dates = CrashReports.recent(process: "SherlockMe", since: now.addingTimeInterval(-7 * 86_400),
    ...
    func testAFolderThatCannotBeReadCountsNothing() {
        XCTAssertEqual(CrashReports.recent(process: "SherlockMe", since: .distantPast,
