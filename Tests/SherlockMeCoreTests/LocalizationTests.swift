...
final class LocalizationTests: XCTestCase {
    ...
    /// Every accessor of every table, for one language. Written out rather than reflected.
    private func everySentence(_ language: Language) -> [(String, String)] {
        ...
        for pair in [("onboarding.pitchHeadline", onboarding.pitchHeadline),
                     ...
                     ("onboarding.pitchMenuBarPill", onboarding.pitchMenuBarPill),
                     ("onboarding.pitchPrivatePill", onboarding.pitchPrivatePill),
        ...
        for pair in [("settings.pageGeneral", settings.pageGeneral),
                     ...
                     ("settings.pageHealth", settings.pageHealth),
                     ("settings.pageTip", settings.pageTip)] { add(pair.0, pair.1) }
        ...
        for pair in [("system.startOverTitle", system.startOverTitle),
    ...
    /// Control and Data Access\u{201D}, and that \u{201C}Control\u{201D} is not the ⌃ Control key. Such a name
    /// is quoted from the pane's own strings and cannot be reworded, so it is exempted one by one, and only
    /// the one word is let past: every other key rule still applies to it.
    ...
    func testTheAppsNameIsNeverSpeltOutInATable() throws {
        let core = URL(fileURLWithPath: #filePath)
            ...
            .appendingPathComponent("Sources/SherlockMeCore")
        ...
        for table in tables {
            ...
            for (number, line) in text.split(separator: "\n", omittingEmptySubsequences: false).enumerated() {
                ...
                guard code.contains("\""), code.contains("SherlockMe"),
