...
/// app. Making the request is `SherlockMePlatform.UpdateChecker`'s and is not exercised here.
final class UpdateCheckTests: XCTestCase {
    private func release(_ version: String) -> LatestRelease {
        LatestRelease(version: ReleaseVersion(string: version)!,
                      dmgURL: URL(string: "https://example.invalid/SherlockMe-\(version).dmg")!)
    }

    private func json(tag: String? = "v1.2.0", assets: [String] = ["SherlockMe-1.2.0.dmg"],
    ...
    func testAReleaseNeedsATagAndADiskImage() {
        ...
        XCTAssertNil(LatestRelease.parse(json(assets: ["SherlockMe.zip"])))
