...
final class UpdateInstallScriptTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        root = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("sherlockme-install-\(UUID().uuidString)", isDirectory: true)
    ...
    private func bundle(_ name: String, marker: String) throws -> URL {
        ...
        let executable = binary.appendingPathComponent("SherlockMe")
    ...
    private func plan(destination: URL, staged: URL) -> UpdateInstallPlan {
        UpdateInstallPlan(pid: 999_999,
                          ...
                          backup: root.appendingPathComponent("previous/SherlockMe.app"),
                          resultFile: root.appendingPathComponent("result"),
                          logFile: root.appendingPathComponent("install.log"),
                          executableName: "SherlockMe", version: "1.2.0",
                          quitWait: 1, launchWait: 1, settle: 0)
    }

    private func marker(at bundle: URL) -> String? {
        try? String(contentsOf: bundle.appendingPathComponent("Contents/MacOS/SherlockMe"), encoding: .utf8)
    }
    ...
    func testTheNewVersionTakesTheOldOnesPlace() throws {
        let destination = try bundle("SherlockMe.app", marker: "old")
        let staged = try bundle("staged/SherlockMe.app", marker: "new")
        let open = try stubOpen()
        let ps = try stubPS(reporting: destination.appendingPathComponent("Contents/MacOS/SherlockMe").path)
        ...
        XCTAssertFalse(FileManager.default.fileExists(
            atPath: root.appendingPathComponent("previous/SherlockMe.app").path))
    ...
    func testAMissingStagedCopyTouchesNothing() throws {
        let destination = try bundle("SherlockMe.app", marker: "old")
        let staged = root.appendingPathComponent("staged/SherlockMe.app")
        let open = try stubOpen()
        let ps = try stubPS(reporting: nil)
    ...
    func testThePlansArgumentsAreInTheOrderTheScriptReadsThem() {
        ...
        XCTAssertEqual(plan.arguments[6], "SherlockMe")
