...
- **The Touch ID key locks, and the Mac stays locked.** A click on the key locks the Mac the instant it goes
  ...
  permission.
- **Touch ID in your other apps is untouched.** While an app or System Settings reads your finger, and for
  3 s after, a click of the sensor does exactly what macOS makes it do: nothing. A click right after you
  unlock with Touch ID still locks. Another user's session on the same Mac is left alone too.
